(function () {
	...
	var Expaste = function (clipboardData, interfaceType) {
		if (this instanceof Expaste) {
			this.init(clipboardData, interfaceType);
		} else {
			return new Expaste(clipboardData, interfaceType);
		}
	};

	Expaste.prototype.init = function (clipboardData, interfaceType) {

		var text, html, img;

		interfaceType = interfaceType || 'w3c';
	...
	Expaste.getDataByEvent = function (e) {
		if (e.clipboardData) {
			...
			return new Expaste(window.clipboardData, 'ms');
		} else {
			return null;
		}
	};
